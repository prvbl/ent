...
func (uq *UserQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if ps := uq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector, entuser.ValidColumn)
			}
		}
	}
	return _spec
...
func (uq *UserQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	selector := builder.Select(t1.Columns(entuser.Columns...)...).From(t1)
	if uq.sql != nil {
		selector = uq.sql
		selector.Select(selector.Columns(entuser.Columns...)...)
	}
	for _, p := range uq.predicates {
		p(selector)
	}
	for _, p := range uq.order {
		p(selector, entuser.ValidColumn)
	}
	if offset := uq.offset; offset != nil {
...
func (ugb *UserGroupBy) sqlQuery() *sql.Selector {
	...
	for _, fn := range ugb.fns {
		columns = append(columns, fn(selector, entuser.ValidColumn))
	}
